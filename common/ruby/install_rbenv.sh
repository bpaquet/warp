...
RBENV_TAG="v0.4.0"
RBENV_RUBY_BUILD_TAG="v20141225"
...
if [ ! -d .rbenv ]; then
  echo "Cloning rbenv"
  run git clone $(process_git_url ${git_protocol}://github.com/sstephenson/rbenv.git) .rbenv
...
if [ ! -d ruby-build ]; then
  run git clone $(process_git_url ${git_protocol}://github.com/sstephenson/ruby-build.git)
fi
...
if [ ! -d rbenv-gemset ]; then
  run git clone $(process_git_url ${git_protocol}://github.com/jamis/rbenv-gemset.git)
fi
