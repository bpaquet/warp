...
RBENV_TAG="v0.4.0"
RBENV_RUBY_BUILD_TAG="v20131008"
RBENV_GEMSET_TAG="v0.4.1"
...
if [ ! -d .rbenv ]; then
  echo "Cloning rbenv"
  run git clone $(process_git_url git://github.com/sstephenson/rbenv.git) .rbenv
...
if [ ! -d ruby-build ]; then
  run git clone $(process_git_url git://github.com/sstephenson/ruby-build.git)
fi
...
if [ ! -d rbenv-gemset ]; then
  run git clone $(process_git_url git://github.com/jamis/rbenv-gemset.git)
fi
